...
class Jatek:
    def __init__(self) -> None:
        ...
        self._pont_felirat_terulete: pygame.Surface = self._betutipus.render(
            "Pont", True, Szinek.feher
        )
        self._kovetkezo_felirat_terulete: pygame.Surface = self._betutipus.render(
            "Következő", True, Szinek.feher
        )
        self._jatek_vege_felirat: pygame.Surface = self._betutipus.render(
            "JATEK VEGE", True, Szinek.feher
        )
    ...
    def frissit(self) -> None:
        for esemeny in pygame.event.get():
            ...
            if esemeny.type == self._jatek_frissites:
                if not self._tetrisz.jatek_vege:
                    self._tetrisz.mozgas_le()

    def rajzol(self) -> None:
        pont_ertek_terulete: pygame.Surface = self._betutipus.render(
            str(self._tetrisz.pontszam), True, Szinek.feher
        )
        ...
        self._ablak.blit(
            pont_ertek_terulete,
            pont_ertek_terulete.get_rect(
                centerx=self._pontok_terulete.centerx,
                centery=self._pontok_terulete.centery,
            ),
        )
        pygame.draw.rect(
            self._ablak, Szinek.vilagoskek, self._kovetkezo_terulete, 0, 10
        )
